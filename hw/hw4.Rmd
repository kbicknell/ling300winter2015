...
* You will analyze the `lexdec` dataset from the `languageR` package that we've been using in class. The dependent measures will be the `RT` column (which is already log-transformed) and (an appropriately transformed version of) the Correct column, your independent measure will be (a centered version of) the Frequency column, and the relevant grouping variables for the random effects are the Subject and Word columns.
